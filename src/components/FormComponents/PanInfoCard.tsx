		...
		{index === 0 && (
			<Grid item xs={12} ml={1} my={1} display={"flex"} justifyContent={"center"} key={`grid${index}`}>
				<Typography variant="body1" fontWeight="600">
					Inserire i dati del pan (opzionali)
				</Typography>
			</Grid>
		)}
		...
		{index === 0 && (
			<Grid container item xs={12} ml={1} my={1} display={"flex"} justifyContent={"flex-start"} key={`button${index}`}>
				<Grid item xs={6}>
					<Button
						size={"small"}
						id="visible-section-btn"
						data-testid="visible-section-btn-test"
						variant="text"
						onClick={() => {
							optionalPaymentMethodPanManagment();
						}}
						disabled={!Object.values(formDataPanInfoCards.panInfo[0])?.every((field) => field.length > 0) && formDataPanInfoCards.panInfo.length <= 1}
					>
						{
